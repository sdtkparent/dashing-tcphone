    SCHEDULER.every '15m', :first_in => 0 do |job|
        builds=config[:builds].map{|currentWaitingCalls|
            currentOldest=JSON(get("http://iaonuccx01.sdt.local:9080/realtime/AgentCSQStats"))[0]['result']?'ok':'failing'
            {:currentWaitingCalls => NumberWaiting, :currentOldest => LongestWaiting}
        }
